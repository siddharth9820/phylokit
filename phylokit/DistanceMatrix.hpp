...
class DistanceMatrix {
 ...
 public:
  ...
  DistanceMatrix(const DistanceMatrix &other)
      : ts(other.ts), d(other.d), mask_(other.mask_) {}

  double &get(Taxon t1, Taxon t2, std::vector<double> &myD);
  double get(Taxon t1, Taxon t2, const std::vector<double> &myD) const;

  double &operator()(Taxon t1, Taxon t2) { return get(t1, t2); }
  double &get(Taxon t1, Taxon t2) { return get(t1, t2, d); }

  double has(Taxon t1, Taxon t2) const { return get(t1, t2, mask_) != 0; }

  double isMasked(Taxon t1, Taxon t2) const { return get(t1, t2, mask_) == 0; }
  double &masked(Taxon t1, Taxon t2) { return get(t1, t2, mask_); }
